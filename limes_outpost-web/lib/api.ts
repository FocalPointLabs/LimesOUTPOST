...
export const venturesApi = {
  ...
  create: (body: VentureCreateRequest) => api.post<Venture>("/ventures", body),
  patch: (id: string, body: VenturePatchRequest) => api.patch<Venture>(`/ventures/${id}`, body),
  deactivate: (id: string) => api.delete(`/ventures/${id}`),
...
export const pipelineApi = {
  ...
  // FIXED: Added /run to match the backend router in pulse.py
  pulse: (ventureId: string) => api.post(`/ventures/${ventureId}/pulse/run`),
};

export const publishApi = {
  trigger: (ventureId: string, platform: string) =>
    api.post(`/ventures/${ventureId}/publish/${platform}`),
  triggerAll: (ventureId: string) =>
    api.post(`/ventures/${ventureId}/publish/all`),
  runEmail: (ventureId: string) => api.post(`/ventures/${ventureId}/email/run`),
  runSocial: (ventureId: string) => api.post(`/ventures/${ventureId}/social/run`),
};
